...
import com.unievents.dto.PayBillDto;
import com.unievents.dto.PayDto;
import com.unievents.dto.RefundDto;
import com.unievents.dto.TradeCheckDto;
import com.unievents.entity.PayBill;
import com.unievents.entity.RefundBill;
...
import com.unievents.mapper.PayBillMapper;
import com.unievents.mapper.RefundBillMapper;
import com.unievents.pay.PayResult;
import com.unievents.pay.PayStrategyContext;
import com.unievents.pay.PayStrategyHandler;
import com.unievents.pay.RefundResult;
import com.unievents.pay.TradeResult;
import com.unievents.servicelock.annotion.ServiceLock;
import com.unievents.util.DateUtils;
import com.unievents.vo.NotifyVo;
import com.unievents.vo.PayBillVo;
...
import static com.unievents.constant.Constant.ALIPAY_NOTIFY_FAILURE_RESULT;
import static com.unievents.constant.Constant.ALIPAY_NOTIFY_SUCCESS_RESULT;
import static com.unievents.core.DistributedLockConstants.COMMON_PAY;
import static com.unievents.core.DistributedLockConstants.TRADE_CHECK;
...
public class PayService {
    ...
    public String commonPay(PayDto payDto) {
        ...
        if (Objects.nonNull(payBill) && !Objects.equals(payBill.getPayBillStatus(), PayBillStatus.NO_PAY.getCode())) {
            throw new unieventsFrameException(BaseCode.PAY_BILL_IS_NOT_NO_PAY);
        }
    ...
    public String refund(RefundDto refundDto) {
        ...
        if (Objects.isNull(payBill)) {
            throw new unieventsFrameException(BaseCode.PAY_BILL_NOT_EXIST);
        }
        
        if (!Objects.equals(payBill.getPayBillStatus(), PayBillStatus.PAY.getCode())) {
            throw new unieventsFrameException(BaseCode.PAY_BILL_IS_NOT_PAY_STATUS);
        }
        
        if (refundDto.getAmount().compareTo(payBill.getPayAmount()) > 0) {
            throw new unieventsFrameException(BaseCode.REFUND_AMOUNT_GREATER_THAN_PAY_AMOUNT);
        }
        ...
        if (refundResult.isSuccess()) {
            ...
            throw new unieventsFrameException(refundResult.getMessage());
        }
    }
