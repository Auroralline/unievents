...
import com.unievents.service.delaysend.DelayOrderCancelSend;
import com.unievents.service.kafka.CreateOrderMqDomain;
import com.unievents.service.kafka.CreateOrderSend;
import com.unievents.service.lua.ProgramCacheCreateOrderData;
import com.unievents.service.lua.ProgramCacheCreateOrderResolutionOperate;
import com.unievents.service.lua.ProgramCacheResolutionOperate;
import com.unievents.service.tool.SeatMatch;
...
import static com.unievents.service.constant.ProgramOrderConstant.ORDER_TABLE_COUNT;
...
public class ProgramOrderService {
        ...
        if (CollectionUtil.isNotEmpty(seatDtoList)) {
            for (SeatDto seatDto : seatDtoList) {
                ...
                if (Objects.nonNull(ticketCategoryVo)) {
                    ...
                    throw new unieventsFrameException(BaseCode.TICKET_CATEGORY_NOT_EXIST_V2);
                }
            }
        } else {
        ...
        if (CollectionUtil.isNotEmpty(seatDtoList)) {
            ...
            for (Entry<Long, Long> entry : seatTicketCategoryDtoCount.entrySet()) {
                ...
                Long remainNumber = Optional.ofNullable(ticketCategoryRemainNumber.get(String.valueOf(ticketCategoryId)))
                        .orElseThrow(() -> new unieventsFrameException(BaseCode.TICKET_CATEGORY_NOT_EXIST_V2));
                if (purchaseCount > remainNumber) {
                    throw new unieventsFrameException(BaseCode.TICKET_REMAIN_NUMBER_NOT_SUFFICIENT);
                }
            }
            ...
            for (SeatDto seatDto : seatDtoList) {
                ...
                if (Objects.isNull(seatVo)) {
                    throw new unieventsFrameException(BaseCode.SEAT_IS_NOT_NOT_SOLD);
                }
            ...
            if (parameterOrderPrice.compareTo(databaseOrderPrice) > 0) {
                throw new unieventsFrameException(BaseCode.PRICE_ERROR);
            }
        }else {
            ...
            if (purchaseSeatList.size() < ticketCount) {
                throw new unieventsFrameException(BaseCode.SEAT_OCCUPY);
            }
        }
    ...
    public List<SeatVo> createOrderOperateProgramCacheResolution(ProgramOrderCreateDto programOrderCreateDto){
        ...
        if (!Objects.equals(programCacheCreateOrderData.getCode(), BaseCode.SUCCESS.getCode())) {
            throw new unieventsFrameException(Objects.requireNonNull(BaseCode.getRc(programCacheCreateOrderData.getCode())));
        }
        ...
        for (int i = 0; i < ticketUserIdList.size(); i++) {
            ...
            SeatVo seatVo =
                    Optional.ofNullable(purchaseSeatList.get(i))
                            .orElseThrow(() -> new unieventsFrameException(BaseCode.SEAT_NOT_EXIST));
    ...
    private String createOrderByRpc(OrderCreateDto orderCreateDto,List<SeatVo> purchaseSeatList){
        ...
        if (!Objects.equals(createOrderResponse.getCode(), BaseCode.SUCCESS.getCode())) {
            ...
            throw new unieventsFrameException(createOrderResponse);
        }
    ...
    private String createOrderByMq(OrderCreateDto orderCreateDto,List<SeatVo> purchaseSeatList){
        ...
        createOrderSend.sendMessage(JSON.toJSONString(orderCreateDto),sendResult -> {
            ...
            log.error("创建订单失败 需人工处理 orderCreateDto : {}",JSON.toJSONString(orderCreateDto));
            updateProgramCacheDataResolution(orderCreateDto.getProgramId(),purchaseSeatList,OrderStatus.CANCEL);
            createOrderMqDomain.unieventsFrameException = new unieventsFrameException(ex);
            latch.countDown();
        });
        try {
            latch.await();
            ...
            throw new unieventsFrameException(e);
        }
        if (Objects.nonNull(createOrderMqDomain.unieventsFrameException)) {
            throw createOrderMqDomain.unieventsFrameException;
        }
